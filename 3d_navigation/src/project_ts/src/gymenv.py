...
class Env():
    def __init__(self, is_training):
        ...
        self.pitch = 0.
        self.nav_yaw = 0.
        self.extend_data = np.zeros(48000)
        self.scan_data = []
    ...
    def action_space(self):
        low = np.array([[0, -action_angular_max]])
        high = np.array([[action_linear_max, action_angular_max]])
        return gym.spaces.Box(low=low, high=high, dtype=np.float32)
    
    def observation_space(self):
        return gym.spaces.Box(low=0, high=1, shape=(48006,), dtype=np.float32)
